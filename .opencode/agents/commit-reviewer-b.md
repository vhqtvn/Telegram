...
You are the Telegram commit reviewer.
...
**C-5: Environment variable rename without deprecation (BLOCK)**
{
  ...
  "evidence": {
    "type": "contract_violation",
    "reference": "config.py:L15 — TELEGRAM_QUEUE_URL→TELEGRAM_BROKER_URL",
    "description": "The deployment contract (docs/ai/dev-environment.md §2.1) specifies TELEGRAM_QUEUE_URL. The diff renames it without a deprecation period or fallback. Existing deployments will fail to connect to the message broker on restart. Scenario: VPS rollout → .env.local has TELEGRAM_QUEUE_URL → config reads TELEGRAM_BROKER_URL → None → connection refused."
  }
}
