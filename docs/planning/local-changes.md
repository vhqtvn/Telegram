...
- keystore: optional `RELEASE_STORE_FILE` gradle property (absolute or repo-root-relative, e.g. in `~/.gradle/gradle.properties` — that file takes precedence over the committed dummy passwords in `gradle.properties`). Real keystores/passwords are NEVER committed.
